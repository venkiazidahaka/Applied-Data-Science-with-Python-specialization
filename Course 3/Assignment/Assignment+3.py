...
def answer_two():
    ...
    accsc = dummy_classifier.score(X_test, y_test) 
    recsc = recall_score(y_test, dummy_predictions)
    answer = (accsc,recsc)
...
def answer_three():
    ...
    accsc = svm.score(X_test, y_test)
    recsc = recall_score(y_test, svm_predicted)
    presc = precision_score(y_test, svm_predicted)
    answer = (accsc,recsc,presc)
...
def answer_six():    
    ...
    grid_rec = GridSearchCV(lr, param_grid = grid_values, scoring = 'recall')
    grid_rec.fit(X_train, y_train)
    
    answer = np.array([grid_rec.cv_results_['mean_test_score'][x:x+2] for x in range(0, len(grid_rec.cv_results_['mean_test_score']), 2)])
    return answer
