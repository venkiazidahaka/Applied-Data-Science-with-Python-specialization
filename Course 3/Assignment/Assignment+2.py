...
def answer_one():
    ...
    ans9 = linreg9.predict(y_poly9)
    
    answer = np.vstack((ans1, ans3, ans6, ans9))
...
def answer_two():
    ...
    for i in range(10):
        ...
        ans_train.append(linreg.score(X_train_poly, y_train))
        ans_test.append(linreg.score(X_test_poly, y_test))

    return np.asarray(ans_train), np.asarray(ans_test)
...
def answer_five():
    ...
    df = pd.DataFrame({'feature': X_train2.columns.values, 'importance': dt.feature_importances_})
    answer = df.sort(['importance'], ascending=[0])['feature'].head(5).tolist()
...
def answer_seven():
    ...
    par_range = np.logspace(-4, 1, 6)
    answer = (par_range[0], par_range[5], par_range[3])
